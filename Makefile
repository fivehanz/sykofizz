...
PROJECT_NAME=sykofizz
STACK_NAME=sykofizz-com
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://sykofizz.localhost"
...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend --build-arg PLONE_VERSION=${PLONE_VERSION} -t fivehanz/sykofizz-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend --build-arg VOLTO_VERSION=${VOLTO_VERSION} -t fivehanz/sykofizz-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name sykofizz-backend-acceptance -d fivehanz/sykofizz-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name sykofizz-frontend-acceptance --link sykofizz-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d fivehanz/sykofizz-frontend:acceptance

.PHONY: stop-acceptance-servers
stop-acceptance-servers: ## Stop Acceptance Servers
	@echo "Stop acceptance containers"
	@docker stop sykofizz-frontend-acceptance
	@docker stop sykofizz-backend-acceptance
