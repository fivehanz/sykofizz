...
2. Go to your repository at [fivehanz/sykofizz](https://github.com/fivehanz/sykofizz).
3. Click on `Settings` at the top-right corner.
4. In the left sidebar, click on `Environments`.
5. Press `New environment`.
6. Name the environment `sykofizz.com` and hit `Configure environment`.
...
|-------------|--------------|-------------|
| DEPLOY_HOST | sykofizz.com | The hostname or IP address of your Docker Swarm manager. |
...
- [Backend Tests Workflow](https://github.com/fivehanz/sykofizz/actions/workflows/backend.yml)
- [Frontend Tests Workflow](https://github.com/fivehanz/sykofizz/actions/workflows/frontend.yml)

Upon successful completion of the tests, Docker images for the Backend (`fivehanz/sykofizz-backend`) and Frontend (`fivehanz/sykofizz-frontend`) will be available.

### Initiating Manual Deployment

1. Navigate to [Manual Deployment of sykofizz.com](https://github.com/fivehanz/sykofizz/actions/workflows/manual-deploy.yml).
...
- Initiate a new deployment using the `devops/stacks/sykofizz.com.yml` stack.
